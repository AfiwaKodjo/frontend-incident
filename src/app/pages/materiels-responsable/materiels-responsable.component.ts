...
@Component({
  selector: 'app-materiels-responsable',
  standalone: true,
...
export class MaterielsResponsableComponent implements OnInit{
  ...
  constructor(private materielsService: MaterielsService, private router: Router, private proceduresServices: ProceduresService) { }
  ...
  private getProcedures() {
    this.proceduresServices.getProcedures().subscribe(data => {
  ...
  console.log(materielModel);
        ...
        (error: HttpErrorResponse) => {
          if (error.status === 500) {
            Swal.fire({
              icon: 'error',
              title: 'Erreur',
              text: 'Suppression non autorisée. Revoyez le client!! '
            });
            this.getMateriels();
          } else  if (error.status === 200) {
            Swal.fire({
    ...
    updateMateriel(idMateriel: number){
      this.router.navigate(['responsable/update-materiels/idMateriel', idMateriel]);
    }   
